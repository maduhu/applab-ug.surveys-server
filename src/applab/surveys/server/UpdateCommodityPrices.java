...
public class UpdateCommodityPrices extends ApplabServlet {
    ...
    private static final long serialVersionUID = 2995154454090041017L;
    ...
    public void doApplabPost(HttpServletRequest request, HttpServletResponse response, ServletRequestContext context) throws Exception {
        ...
        performSubmissionProcessing();
        // }
        // catch (Exception ex) {
        // log("Failed to complete processing of market submissions" );
        // ex.printStackTrace();
        // }
    }
    ...
    public void doApplabGet(HttpServletRequest request, HttpServletResponse response, ServletRequestContext context) throws Exception {
        ...
        calendar.add(Calendar.DATE, -1);
        marketDay = DateHelpers.getDayOfWeek(calendar.getTime());
        log("Processing for Market Day: " + marketDay);
        performSubmissionProcessing();
        // }
        // catch (Exception ex) {
        // log("Failed to complete processing of market submissions" + ex.getMessage());
        // }
    ...
    public ArrayList<Survey> loadSurveys(Set<String> salesforceSurveyIds) throws Exception {
        ...
        for (String salesforceSurveyId : salesforceSurveyIds) {
            try {
                ...
                surveys.add(survey);
                log("Loaded Survey: " + survey.getName());
            }
            catch (Exception ex) {
    ...
    public boolean saveCommodityPricesToBackendDataBase(ArrayList<Commodity> commodities)
        ...
        for (Commodity commodity : commodities) {
            ...
            log("My SQL insert statement: " + submissionStatement.toString());
        }
        try {
        ...
        for (Survey survey : surveys) {
            ...
            for (String questionBinding : parsedSurveyXml.getQuestionOrder()) {
                ...
                if (isCommodityPriceRelatedBinding(questionBinding)) {
                    log("Question Binding: " + questionBinding);
